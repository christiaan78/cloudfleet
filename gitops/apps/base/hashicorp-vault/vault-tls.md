...
1. **All in-cluster traffic to Vault is HTTPS.**
   Clients inside Kubernetes (ESO, apps, CLI, probes) connect to
   `https://hashicorp-vault.apps.svc:8200`, and **validate** Vault’s identity.
...
* Using that CA, cert-manager issues a **server certificate** for Vault with DNS SANs that match the service names:
  ...
  * `hashicorp-vault.apps.svc`
  * `hashicorp-vault.apps.svc.cluster.local`
* cert-manager stores:

  * the **CA** in a Secret (`apps/vault-internal-ca`)
  * the **server cert** in a Secret (`apps/vault-server-tls`)
...
* Traefik gets a **ServersTransport** object that says:
  ...
  * “When I call Vault, use SNI `hashicorp-vault.apps.svc` and trust this **CA**.”
...
* `gitops/infrastructure/base/tls/vault/`
  ...
  * `certificate-internal-ca.yaml` (**creates** `apps/vault-internal-ca` – the CA)
  * `issuer-internal-ca.yaml` (Issuer backed by that CA)
  * `certificate-vault-server.yaml` (**creates** `apps/vault-server-tls` – Vault server cert)
  * `kustomization.yaml`
...
* `gitops/apps/base/hashicorp-vault/`

  * `helmrepository.yaml` (HashiCorp charts)
  * `helmrelease.yaml` (chart `vault`)
  * `kustomization.yaml`
* `gitops/apps/development/hashicorp-vault/`
...
* `gitops/apps/base/traefik/`

  * `helmrelease-traefik.yaml`, `helmrelease-crds.yaml`, `helmrepository.yaml`, `kustomization.yaml`
* `gitops/apps/development/traefik/`

  * `kustomization.yaml`, `namespace.yaml`
  * `serverstransport-vault.yaml` (**ServersTransport** with `serverName: hashicorp-vault.apps.svc` and `rootCAs: - secret: vault-ca`)
  * `vault-ca-secret.yaml` (**static** Secret with `ca.crt` for Traefik to trust the Vault CA)
  * `patch-helmrelease-values.yaml` (enables `providers.kubernetesCRD.allowCrossNamespace: true` so the Service in `apps` can reference a ServersTransport in `ingress-traefik`)

> **Why cross-namespace?**
> Our Service (in `apps`) points to a ServersTransport located in `ingress-traefik`. The Traefik chart flag `providers.kubernetesCRD.allowCrossNamespace: true` enables that reference.
...
* `gitops/clusters/development/`
  ...
  * `apps-hashicorp-vault-development.yaml`
  * `apps-traefik-development.yaml`
  * `apps-kube-prometheus-stack-development.yaml`
  * `apps-podinfo-development.yaml`

Order is enforced with `dependsOn` (or by sequencing the infra Kustomizations before the apps).
...
1. Client resolves `hashicorp-vault.apps.svc` via kube-DNS.
...
2. Traefik routes to Service `hashicorp-vault` in `apps`.
3. Because the Service carries **Traefik service annotations**, Traefik:

   * uses **HTTPS upstream**
   * uses the **ServersTransport** `vault-tls` (SNI `hashicorp-vault.apps.svc`, trust bundle **`vault-ca`** Secret)
...
* **Secrets exist**
  `apps/vault-internal-ca` (CA) and `apps/vault-server-tls` (server) both present with data; Traefik’s `ingress-traefik/vault-ca` has **`ca.crt`**.
...
* **Service annotations present**
  `kubectl -n apps get svc hashicorp-vault -o yaml` shows:
...
* **ServersTransport present**
  ...
  * `serverName: hashicorp-vault.apps.svc`
...
* **Not enabling cross-namespace CRD references in Traefik.**
  Without `providers.kubernetesCRD.allowCrossNamespace: true`, a Service in `apps` cannot use a ServersTransport in `ingress-traefik`.
...
* **Vault server cert missing the right SANs.**
  The server Certificate **must** include `hashicorp-vault.apps.svc` (and `.svc.cluster.local`). Otherwise SNI validation fails.
